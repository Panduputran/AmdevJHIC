    ...
    <div class="bg-white rounded-lg shadow-md p-6">
        ...
        <form action="{{ route('admin.teachers.update', $teacher->id) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="mb-4">
                ...
                <input type="text" name="name" id="name" class="w-full px-4 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-[#6CF600] @error('name') border-red-500 @enderror" value="{{ old('name', $teacher->name) }}">
                @error('name')
            ...
            <div class="mb-4">
                ...
                @if($teacher->photo)
                    ...
                    <img src="{{ asset('storage/' . $teacher->photo) }}" alt="Foto {{ $teacher->name }}" class="w-32 h-32 object-cover rounded-md mt-2">
                @endif
                @error('photo')
                    <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                @enderror
            ...
            <div class="mb-4">
                ...
                <input type="text" name="position" id="position" class="w-full px-4 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-[#6CF600] @error('position') border-red-500 @enderror" value="{{ old('position', $teacher->position) }}">
                @error('position')
                    <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="mb-4">
                ...
                <input type="text" name="subject" id="subject" class="w-full px-4 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-[#6CF600] @error('subject') border-red-500 @enderror" value="{{ old('subject', $teacher->subject) }}">
                @error('subject')
                    <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="mb-4">
                ...
                <select name="school" id="school" class="w-full px-4 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-[#6CF600] @error('school') border-red-500 @enderror">
                    <option value="Amaliah 1" @if(old('school', $teacher->school) == 'Amaliah 1') selected @endif>Amaliah 1</option>
                    <option value="Amaliah 2" @if(old('school', $teacher->school) == 'Amaliah 2') selected @endif>Amaliah 2</option>
                </select>
                @error('school')
                    <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                @enderror
            ...
            <div class="mb-6">
                ...
                <select name="category" id="category" class="w-full px-4 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-[#6CF600] @error('category') border-red-500 @enderror">
                    <option value="Produktif" @if(old('category', $teacher->category) == 'Produktif') selected @endif>Produktif</option>
                    <option value="Normatif" @if(old('category', $teacher->category) == 'Normatif') selected @endif>Normatif</option>
                    <option value="Adaptif" @if(old('category', $teacher->category) == 'Adaptif') selected @endif>Adaptif</option>
                    <option value="Umum" @if(old('category', $teacher->category) == 'Umum') selected @endif>Umum</option>
                </select>
                @error('category')
                    <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                @enderror
