    ...
    <div class="bg-white rounded-lg shadow-md p-6">
        ...
        <form action="{{ route('admin.majors.update', $major->id) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="mb-4">
                ...
                <input type="text" name="name" id="name" class="w-full px-4 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-[#6CF600] @error('name') border-red-500 @enderror" value="{{ old('name', $major->name) }}">
                @error('name')
            ...
            <div class="mb-4">
                ...
                <textarea name="description" id="description" rows="5" class="w-full px-4 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-[#6CF600] @error('description') border-red-500 @enderror">{{ old('description', $major->description) }}</textarea>
                @error('description')
                    <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="mb-4">
                ...
                @if($major->image)
                    ...
                    <img src="{{ asset('storage/' . $major->image) }}" alt="Gambar Jurusan" class="w-32 h-32 object-cover rounded-md mt-2">
                @endif
                @error('image')
                    <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                @enderror
            ...
            <div class="mb-4">
                ...
                <input type="text" name="competency_head" id="competency_head" class="w-full px-4 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-[#6CF600] @error('competency_head') border-red-500 @enderror" value="{{ old('competency_head', $major->competency_head) }}">
            ...
            <div class="mb-6">
                ...
                @if($major->competency_head_photo)
                    ...
                    <img src="{{ asset('storage/' . $major->competency_head_photo) }}" alt="Foto Kepala Kompetensi" class="w-32 h-32 object-cover rounded-md mt-2">
                @endif
