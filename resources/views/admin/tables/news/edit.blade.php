    ...
    <div class="bg-white rounded-lg shadow-md p-6">
        ...
        <form action="{{ route('admin.news.update', $newsItem->id) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="mb-4">
                ...
                <input type="text" name="title" id="title" class="w-full px-4 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-[#6CF600] @error('title') border-red-500 @enderror" value="{{ old('title', $newsItem->title) }}">
                @error('title')
            ...
            <div class="mb-4">
                ...
                @if($newsItem->image)
                    ...
                    <img src="{{ asset('storage/' . $newsItem->image) }}" alt="Gambar Berita" class="w-32 h-32 object-cover rounded-md mt-2">
                @endif
                @error('image')
                    <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                @enderror
            ...
            <div class="mb-4">
                ...
                <textarea name="description" id="description" rows="5" class="w-full px-4 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-[#6CF600] @error('description') border-red-500 @enderror">{{ old('description', $newsItem->description) }}</textarea>
                @error('description')
                    <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="mb-6">
                ...
                <input type="date" name="date_published" id="date_published" class="w-full px-4 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-[#6CF600] @error('date_published') border-red-500 @enderror" value="{{ old('date_published', $newsItem->date_published) }}">
